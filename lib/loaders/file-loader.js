...
const getFileLoaderOptions = ({
  ...
  let publicPath = `/_next/public/${imagesFolder}/`;

  if (imagesPublicPath) {
    publicPath = imagesPublicPath;
  } else if (assetPrefix) {
    publicPath = `${assetPrefix}${assetPrefix.endsWith('/') ? '' : '/'}_next/public/${imagesFolder}/`;
  }

  return {
    publicPath,
    outputPath: imagesOutputPath || `${isServer ? '../' : ''}public/${imagesFolder}/`,
    name: imagesName,
  };
};
