...
const isModuleInstalled = (name) => {
  try {
    ...
    return module && !module.startsWith(path.resolve(__dirname, '..', '..', 'node_modules'));
  } catch (e) {
    return false;
  }
};
...
const getNumLoadersInstalled = loaders => Object.values(loaders).filter(Boolean).length;
