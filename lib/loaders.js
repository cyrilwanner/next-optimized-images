...
export const appendLoaders = (webpackConfig, nextConfig, detectedLoaders) => {
  ...
  if (detectedLoaders.jpeg || detectedLoaders.png || detectedLoaders.gif || detectedLoaders.svg) {
    config = applyImgLoader(webpackConfig, nextConfig, detectedLoaders, true);
  } else if (handledImageTypes.jpeg) {
    config = applyImgLoader(webpackConfig, nextConfig, detectedLoaders, false);
  }

  if (detectedLoaders.webp) {
    config = applyWebpLoader(webpackConfig, nextConfig, detectedLoaders, true);
  } else if (handledImageTypes.webp) {
    config = applyWebpLoader(webpackConfig, nextConfig, detectedLoaders, false);
  }
